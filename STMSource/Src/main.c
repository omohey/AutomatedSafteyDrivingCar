...
/* Private user code ---------------------------------------------------------*/
/* USER CODE BEGIN 0 */
uint8_t data[95][160]; // Image to be received from the camera
uint8_t right = 0xCA;  // Right motor command
uint8_t left = 0xC2;   // Left motor command
uint8_t speed = 0x20;
...
int main(void)
{
  ...
  HAL_TIM_Base_Start(&htim1); // Start the timer for ultrasonic sensor
  ...
  while (1)
  {
    ...
    HAL_UART_Transmit(&huart1, (uint8_t *)&c, 1, HAL_MAX_DELAY); // Send 'c' to the camera to start transmission
    for (int i = 0; i < 95; i++)
    {
      HAL_UART_Receive(&huart1, data[i], 160, HAL_MAX_DELAY); // Receive the image from the camera  row by row
    }
    ...
    for (int i = 94; i >= 30; i--) // Getting the left and right tire coordinates from the image (using fish eye camera)
    {
      int yindexLeft = -0.4 * i + 90; // Left tire
      int yindexRight = 0.4 * i + 85; // Right tire
